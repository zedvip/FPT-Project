...
public class OrdersListController : ControllerBase
{
    ...
    public OrdersListController(IOrderListService service)
    {
        _service = service;
    }
    ...
    // POST: api/orders
    [HttpPost]
    public async Task<IActionResult> CreateOrder([FromBody] OrderList order)
    {
    ...
    // PUT: api/orders/{id}
    [HttpPut("{id}")]
    public async Task<IActionResult> UpdateOrder(int id, [FromBody] OrderList order)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
